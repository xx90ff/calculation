...
class db
{
    ...
    */
	...
	public static function ins($arg0 = '', $arg1 = NULL, $arg2 = FALSE)
    {
        $arg1 = self::strsafe($arg1);
        $arg1 = self::get_fields($arg0, $arg1);
        $v0 = $v1 = '';
        foreach ($arg1 as $v2 => $v3) {
            $v0 .= "`{$v2}`,";
            $v1 .= "\"{$v3}\",";
        }
        $v4 = "INSERT INTO `{$arg0}` (" . substr($v0, 0, -1) . ') VALUES (' . substr($v1, 0, -1) . ')';
        if ($arg2) {
            exit($v4);
        }
        if (self::query($v4)) {
