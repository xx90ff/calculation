...
/* $Id: Smarty.class.php 3 2009-11-05 07:14:47Z lx $ */
 ...
 */
    ...
    function Smarty()
    {
      //BLT EDIT 不必再兼容php4类的长环境变量
      $this->assign_by_ref('request', req::$forms);
      $this->assign('SCRIPT_NAME', $_SERVER['SCRIPT_NAME']);    
      $this->_supers['server'] = $_SERVER;
      $this->_supers['cookies'] = $_COOKIE;
      if(isset($_SESSION))
      {
        $this->_supers['session'] = $_SESSION;
      }
      //禁止直接调用smarty.get、smarty.post、smarty.request变量
      //$this->_supers['env'] = $_ENV;
    ...
    {
        $is_rewrite = false;
        if(isset($GLOBALS['config']['use_rewrite']) && 
           $GLOBALS['config']['use_rewrite']==true && $GLOBALS['config']['rewrite_rptype']==1)
        {
            $is_rewrite = true;
        }
        if( $is_rewrite )
        ...
        if( $is_rewrite )
        {
            ...
            ob_end_clean();
            cls_rewrite::convert_html($content, 1);
            echo $content;
            $content = '';
        }
    }
    ...
    /**
     * not display, get content html (also this use make html file)
     * @param string $resource_name
     */
    function get_html($resource_name, $cache_id = null, $compile_id = null)
    {
        ob_start();
        $this->fetch($resource_name, $cache_id, $compile_id, true);
        $content = ob_get_contents();
